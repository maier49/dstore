define([
	'intern!object',
	'intern/chai!assert',
	'src/db/has!indexeddb?src/db/IndexedDB',
	'src/db/has!sql?src/db/SQL',
	'src/db/LocalStorage',
	'src/LocalDB',
	'dojo/promise/all',
	'dojo/_base/array',
	...
	function testsForDB(name, DB) {
		...
		return {
			...
			"in('mappedTo', letterStore.filter({vowel: true}).select('id'))": (function() {
				return testQuery(new Filter().in('mappedTo', letterStore.filter({vowel: true}).select('id')),
